...
let concString = hello.concat(" ", myName);
console.log(`New concatenated string: ${concString}`);

// Use the trim method to remove the spaces in anotherString:
let trimAnother = anotherString.trim();
console.log(`New trimmed string: ${trimAnother}`);

// Use the replace method to change “is a” to an empty space in myString:
let replMyString = myString.replace("is a", " ");
console.log(`New replaced string: ${replMyString}`);

// Use the split method on myString using the spaces to break the string:
let myStringSpl = myString.split(" ");
console.log(`myString converted: ${myStringSpl}`);